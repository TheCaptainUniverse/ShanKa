...
$lines.Add("- Run ``bun run release:manual-text-smoke`` to validate the isolated manual text test launcher.")
$lines.Add("- Run ``bun run release:install-smoke`` on Windows before publishing.")
$lines.Add("- Run ``bun run release:msi-smoke`` to validate MSI metadata; rerun from elevated PowerShell with ``-RequireInstall`` for full MSI install smoke.")
